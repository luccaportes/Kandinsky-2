...
class Kandinsky2_1:
    ...
    def __init__(
        ...
        self.prior.load_state_dict(torch.load(prior_path, map_location=torch.device('cpu')), strict=False)
        if self.use_fp16:
        ...
        if self.config["image_enc_params"] is not None:
            ...
            if self.config["image_enc_params"]["name"] == "AutoencoderKL":
                ...
                self.image_encoder.load_state_dict(
                    torch.load(self.config["image_enc_params"]["ckpt_path"], map_location=torch.device('cpu'))
                )
